...
import Util.APIPayloadConstants;
...
//------------------------------------------------------------------------------
    ...
    public void a_get_call_is_made_to_get_all_the_job_titles() {
        response = request.when().get(APIConstants.GET_ALL_JOB_TITLE);
        response.prettyPrint();
    }
    @Then("the status code for the request is {int}")
    public void the_status_code_for_the_request_is(Integer statusCode) {
    response.then().assertThat().statusCode(statusCode);
    }

    @And("the job ID {string} and jobs {string} is stored for other job titles")
    public void theJobIDAndJobsIsStoredForOtherJobTitles(String id, String job) {
        //job_id = response.jsonPath().getString(id);
        job_id = response.jsonPath().getString(id);
        jobs = response.jsonPath().getString(job);
        System.out.println(job_id+" "+jobs);
    }
    //----------------------------------------------------------
